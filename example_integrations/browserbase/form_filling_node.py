...
class FormFillingNode(ReasoningNode):
    ...
    def __init__(
        ...
        logger.info(
            f"FormFillingNode initialized for form: {form_url}"
        )
    ...
    async def cleanup_and_submit(self):
        ...
        if (not self.form_submitted and self.collected_data and
                self.stagehand_filler):
            logger.info(
                "Call ending - auto-submitting form with collected data"
            )
            try:
    ...
    async def _initialize_browser(self):
        ...
        if self.browser_initializing or self.stagehand_filler:
            logger.info(
                "Browser already initializing or initialized, skipping"
            )
            return
    ...
    def _create_questions(self) -> list[FormQuestion]:
        ...
        form_questions = [
            ...
            FormQuestion(
                ...
                question=(
                    "What's your availability - temporary, part-time, "
                    "or full-time?"
                ),
                field_type="radio",
                required=True
    ...
    async def _fill_form_field_async(self, field_name: str, value: str):
        ...
        try:
            ...
            if self.browser_init_task:
                logger.info(
                    f"Waiting for browser to initialize before filling "
                    f"{field_name}"
                )
                await self.browser_init_task
            
            logger.info(
                f"Filling field '{field_name}' with: {value} in background"
            )
            # Use StagehandFormFiller's fill_field method which
            # handles the mapping
            success = await self.stagehand_filler.fill_field(
                field_name, value
            )

            if success:
                logger.info(
                    f"Successfully filled field: {field_name} in browser"
                )
            else:
    ...
    async def _submit_form(self):
        ...
        if self.browser_init_task and not self.stagehand_filler:
            logger.info(
                "Waiting for browser to initialize before submitting form"
            )
            await self.browser_init_task
        ...
        try:
            ...
            self.stagehand_filler.collected_data.update(
                self.collected_data
            )
    ...
    async def process_context(
        ...
        question_name = (
            current_question.field_name if current_question else 'None'
        )
        logger.info(f"Current question: {question_name}")
        logger.info(
            f"Question index: {self.current_question_index}/"
        ...
        is_agent_response = (
            isinstance(latest_event, AgentResponse) if latest_event
            else False
        )
        ...
        # first question
        if (len(context.events) == 2 and not is_agent_response and
                self.current_question_index == 0):
            user_message = context.get_latest_user_transcript_message()
            if user_message and current_question:
                logger.info(f"User ready to start: '{user_message}'")
                logger.info(
                    f"Asking first question: "
                    f"{current_question.field_name}"
                )
                yield AgentResponse(
                    content=f"Great! Let's begin. "
        ...
        if (not current_question and self.current_question_index > 0 and
                len(self.collected_data) > 0):
            # All questions answered - submit the form
            logger.info(
                f"All {self.current_question_index} questions answered"
            )
            logger.info(
                f"Collected data for {len(self.collected_data)} fields"
            )
            ...
            if submission_success:
                goodbye = (
                    "Perfect! I've submitted your application. Thank you!"
                )
            else:
                goodbye = (
        ...
        if not current_question and self.current_question_index == 0:
            logger.warning(
                "No questions available or not properly initialized"
            )
            return
            ...
            if msg.function_calls:
                for function_call in msg.function_calls:
                    if function_call.name == RecordFormFieldTool.name():
                        ...
                        # (non-blocking)
                        asyncio.create_task(
                            self._fill_form_field_async(field_name, value)
                        )
                        ...
                        if next_question:
                            yield AgentResponse(
                                content=f"Great! {next_question.question}"
                            )
