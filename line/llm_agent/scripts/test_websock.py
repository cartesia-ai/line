#!/usr/bin/env python3
"""
Test script for the WebSocketProvider — OpenAI Responses API over WebSocket.
...
Usage:
    uv run python line/llm_agent/scripts/test_websock.py [OPTIONS]
...
async def test_streaming(api_key: str, model: str):
    ...
    provider = WebSocketProvider(model=model, api_key=api_key)
...
async def test_divergence(api_key: str, model: str):
...
async def test_latency_comparison(
    ...
    print_header(f"Latency Comparison: HTTP ({http_model}) vs WS ({ws_model})")
    print(f"Runs: {runs} | Diverge rate: {diverge_rate}")

    http_provider = LLMProvider(model=http_model, api_key=api_key)
    ws_provider = WebSocketProvider(model=ws_model, api_key=api_key)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Test WebSocketProvider (OpenAI Responses API over WS).",
...
async def main(args):
    ...
    try:
        ...
        if "divergence" in tests_to_run:
            await test_divergence(api_key, args.model)
