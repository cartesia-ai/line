...
def _merge_tools(base_tools: Optional[List[Any]], override_tools: Optional[List[Any]]) -> List[Any]:
    ...
    if not base_tools:
        return override_tools or []
    if not override_tools:
        return base_tools

    override_names = {_get_tool_name(t) for t in override_tools}
    filtered_base = [t for t in base_tools if _get_tool_name(t) not in override_names]
