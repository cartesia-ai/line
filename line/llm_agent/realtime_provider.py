...
from line.llm_agent.stream import _iterate_ws_query, _message_identity, _WsEventStream
...
class _RealtimeProvider:
    ...
    async def _send_session_update(self, op: _SessionUpdateOp) -> None:
        ...
        self._state.temperature = op.temperature
        self._state.max_tokens = op.max_tokens
    ...
    async def warmup(
        ...
        *,
        ...
        async with self._lock:
            await self._ensure_connected()
            if config.system_prompt or tool_defs:
...
def _message_to_items(msg: Message) -> List[tuple]:
    """Convert a Message to Realtime API conversation items with identities.

    Returns a list of ``(item_dict, identity_tuple)`` pairs.  A single
    ``Message`` may expand to multiple items — e.g. an assistant message
    with parallel tool calls becomes one ``function_call`` item per call.
    """
    if msg.role == "user":
        item = {
            "type": "message",
...
def _diff_messages(
    ...
    *,
    ...
    needs_session_update = (
        desired_instructions != current_state.system_instructions
        or tool_defs != current_state.tool_defs
        or config.temperature != current_state.temperature
        or config.max_tokens != current_state.max_tokens
    )
    ...
    for msg in non_system:
        desired_pairs.extend(_message_to_items(msg))

    current_items = current_state.items
    desired_identities = [p[1] for p in desired_pairs]
    ...
    to_create = desired_pairs[prefix_len:]
