...
class _RealtimeProvider:
    ...
    def chat(
        ...
        **kwargs,
        ...
        connection setup and diff-sync are issued in ``__aenter__``.  The
    ...
    async def _setup_chat(
        ...
        try:
            ...
            ops = _diff_messages(self._state, messages, tools, config)
            await self._execute_diff(ops, messages, tools, config)
    ...
    async def warmup(
        ...
        async with self._lock:
            await self._ensure_connected()
            if config is not None and (config.system_prompt or tool_defs):
